...
import com.pharmacopoeia.view.cycleView.effect.DefaultTransformer;
...
public class ImageCycleView extends LinearLayout {
     ...
     * 图片轮播视图
     */
    private MViewPage mAdvPager = null;
    /**
     * 滚动图片视图适配
     */
    ...
    public void init(Context context) {
        ...
        LayoutInflater.from(context).inflate(R.layout.home_view_cycle_view, this);
        mAdvPager = (MViewPage) findViewById(R.id.adv_pager);
        ...
        mAdvPager.setPageTransformer(true, new DefaultTransformer());
        // 滚动图片右下指示器视
    ...
    public void setImageResources(ArrayList<String> imageDesList, ArrayList<String> imageUrlList, ImageCycleViewListener imageCycleViewListener) {
        ...
        for (int i = 0; i < imageCount; i++) {
            ...
            if (i == 0) {
                mImageViews[i].setBackgroundResource(R.drawable.home_header_cycle_selected_bg);//.setBackgroundResource(R.drawable.banner_dot_focus);
            } else {
                mImageViews[i].setBackgroundResource(R.drawable.home_header_cycle_dis_selected_bg);
    ...
    public void clear() {
        if (mImageDescList != null) {
            stopImageTimerTask();
            if (mAdvAdapter != null) {
                mAdvAdapter.setSetBoolean(true);
    ...
    private Runnable mImageTimerTask = new Runnable() {
        ...
        public void run() {
            if (mImageViews != null) {
                mImageIndex=mAdvPager.getCurrentItem() + 1;
                mAdvPager.setCurrentItem(mImageIndex);
