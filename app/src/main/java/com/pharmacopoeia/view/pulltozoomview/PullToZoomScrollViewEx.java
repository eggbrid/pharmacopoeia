package com.pharmacopoeia.view.pulltozoomview;
...
public class PullToZoomScrollViewEx extends PullToZoomBase<ScrollView> {
    public interface OnPullToZoomScrollViewExChangedListener {
        public void onScrollChanged(int left, int top, int oldLeft, int oldTop);

        public void pullHeaderToZoom(int newScrollValue);

        public void startAnimation(int paramLong);
    ...
    private OnPullToZoomScrollViewExChangedListener pullToZoomScrollViewExChangedListener;

    public void setPullToZoomScrollViewExChangedListener(OnPullToZoomScrollViewExChangedListener pullToZoomScrollViewExChangedListener) {
        this.pullToZoomScrollViewExChangedListener = pullToZoomScrollViewExChangedListener;
    }
            ...
            public void onInternalScrollChanged(int left, int top, int oldLeft, int oldTop) {
                if (isPullToZoomEnabled() && isParallax()) {
                    ...
                    if ((f > 0.0F) && (f < mHeaderHeight)) {
                        //int i = (int) (0.65D * f);
                        int i = (int) (0.0D * f);//图片和scrollview
    ...
    protected void pullHeaderToZoom(int newScrollValue) {
        this.newScrollValue = newScrollValue;
        //Log.d(TAG, "pullHeaderToZoom --> newScrollValue = " + newScrollValue);
        //Log.d(TAG, "pullHeaderToZoom --> mHeaderHeight = " + mHeaderHeight);
        if (pullToZoomScrollViewExChangedListener != null) {
            pullToZoomScrollViewExChangedListener.pullHeaderToZoom(newScrollValue);
        }
        if (!isRefresh && Math.abs(newScrollValue) > 200) {
            isRefresh = true;
        }
    ...
    protected void smoothScrollToTop() {
        ...
        if (pullToZoomScrollViewExChangedListener != null) {
            pullToZoomScrollViewExChangedListener.startAnimation(newScrollValue);
        }
        mScalingRunnable.startAnimation(30L);//回缩动画时间
    }
        ...
        public void run() {
            if (mZoomView != null) {
                ...
                if ((!mIsFinished) && (mScale > 1.0D)) {
                    ...
                    if (pullToZoomScrollViewExChangedListener != null) {
                        pullToZoomScrollViewExChangedListener.onStopScaling();
                    }
                    if (isRefresh) {
                        if (pullToZoomScrollViewExChangedListener != null) {
                            pullToZoomScrollViewExChangedListener.onHeaderRefresh();
                        }
                    }
                    isRefresh = false;
                    //L.d("缩小动画完成");
    ...
    protected class InternalScrollView extends ScrollView {
        ...
        protected void onScrollChanged(int l, int t, int oldl, int oldt) {
            ...
            if (pullToZoomScrollViewExChangedListener != null) {
                pullToZoomScrollViewExChangedListener.onScrollChanged(l, t, oldl, oldt);
            }
        }
    }
